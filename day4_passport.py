import re
sample = """pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
def valid_passports():
    ...
    passport_attributes = set(['byr','iyr','eyr','hgt','hcl','ecl','pid'])
    ...
    # split by blank line -- a new line without any char/empty line
    passports = passport_data.split('\n\n')
    
    for passport in passports:
        
        passport_detail = re.split(' |\n', passport)
        
        if passport_detail:
            for attr in passport_detail:
                if attr:
            ...
            if passport_valid(passport_dict):
                passport_lst.append(passport_dict)
...
def passport_valid(passport):
    ...
    #     passport_detail = re.split(' |\n', passport) #split by space or new line of each passport using regex
    #     # passport_detail = ['ecl:gry', 'pid:860033327', 'eyr:2020', 'hcl:#fffffd', 'byr:1937', 'iyr:2017', 'cid:147', 'hgt:183cm']
    #     for attr in passport_detail:
            
    #         if attr.split(':')[0] in passport_attributes:
    #             counter +=1
    #             

    #     if len(passport_attributes) == counter:
...
def passport_validation(passport_lst):
    ...
    for passport in passport_lst:
        ...
        # print(f"{passport['hgt']} - {valid_hgt}")
        # # print(f'{hcl} - {valid_hcl}')

        if valid_hcl and valid_byr and valid_iyr and valid_eyr and valid_ecl and valid_pid and valid_hgt:
            valid_passports +=1
            # print(passport)
    ...
    #     # re.search(r"#([0-9]|[a-f]){6}", passport['hcl']):
    #     # "#" followed exactly by 6 digits of 0-9 or a-f
        
    #     if re.match(r"^#([0-9]|[a-f]){6}", passport['hcl']) and len(passport['hcl']) == 7: 
    #         print((re.search(r"^#([0-9]|[a-f]){6}", passport['hcl'])).group())
    #         valid_hcl = True

        
        
    #     # if (valid_byr and valid_iyr and valid_eyr and valid_ecl and valid_pid and valid_hcl and valid_hgt):
    #     if valid_hcl:
    #         print(passport)
    #         # valid_passports += 1
    # # return valid_passports
